...
class RemoteDebtListFragmentViewModel @Inject constructor(
    ...
    fun completeDebt(debt: RemoteDebt) {
        val status = if (debt.role == DebtRole.DEBTOR) WAIT_FOR_COMPLETE_FROM_CREDITOR else WAIT_FOR_COMPLETE_FROM_DEBTOR
    ...
    private fun getDebtStatus(action: RemoteDebtListAdapter.Actions, currentStatus: Int) = when (action) {
